...
def get_map():
    try:
        ...
        inverted_matrix = []
        for row in ground_map.get_grid_for_floor(0).tolist():
            inverted_row = [1 if cell == ground_map.WALKABLE else 0 for cell in row]
            inverted_matrix.append(inverted_row)
        return jsonify({"status": "success", "grid": inverted_matrix})
