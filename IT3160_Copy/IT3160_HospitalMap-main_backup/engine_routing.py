        ...
        for dr, dc, dz in directions:
            ...
            if str(nz) in grid_matrix:
                ...
                if 0 <= nr < len(floor_grid) and 0 <= nc < len(floor_grid[0]):
                    ...
                    # Since 0 is WALKABLE in HospitalMap but in grid format sent by app.py:
                    # inverted_matrix has 1 for Walkable, 0 for Obstacle
                    # Wait, in app.py api_route, grid is NOT inverted!
                    # `grid[str(z)] = hospital_map.get_grid_for_floor(z).tolist()` returns 0 for Walkable and 1 for Obstacle.
                    # Oh, so cost == 0 means Walkable! Let's handle it correctly.
                    if cost is not None and (cost == 0 or cost == 2): 
